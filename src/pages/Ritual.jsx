  ...
  return (
    <button onClick={onClick}
      className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm transition-colors ${
        active ? 'bg-accent text-white border-accent' : 'bg-white text-gray-600 border-gray-200 hover:border-accent hover:text-accent'
      }`}>
      <span>{label}</span>
    ...
    <div className="mt-3 pt-3 border-t border-gray-100">
      ...
      <div className="flex gap-2">
        ...
        <input value={body} onChange={(e) => setBody(e.target.value)}
          ...
          className="flex-1 text-sm border border-gray-200 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-accent/30 min-w-0" />
...
function AccountRow({ account, authorName, onToggleContacted, onRefresh }) {
  ...
  return (
    <div className={`bg-white rounded-xl ring-1 ring-gray-100 shadow-sm overflow-hidden transition-opacity ${account.contacted ? 'opacity-55' : ''}`}>
      <div className="px-4 py-3 flex items-start gap-3">
        <div className="pt-0.5 shrink-0">
          <input type="checkbox" checked={account.contacted} onChange={() => onToggleContacted(account)}
            className="w-4 h-4 rounded border-gray-300 text-accent focus:ring-accent/30 cursor-pointer" title="Mark as contacted" />
        </div>
        <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <a href={account.hubspot_url} target="_blank" rel="noreferrer"
            className="text-xs text-gray-400 hover:text-accent" title="Open in HubSpot">↗</a>
  ...
  return (
    <div className="max-w-screen-lg mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        {hasFilters && <button onClick={() => { setFilterCsam(''); setFilterRag(''); setFilterSignal(''); setSearch(''); }}
